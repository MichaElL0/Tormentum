...
public class FirstPersonAudio : MonoBehaviour
{
	...
	public AudioSource stoneStepAudio;
	...
	AudioSource[] MovingAudios => new AudioSource[] { stepAudio, runningAudio, crouchedAudio, stoneStepAudio };
	...
	void Reset()
	{
		...
		stoneStepAudio = GetOrCreateAudioSource("Stone Audio");
	...
	void FixedUpdate()
	{
		var raycast = Physics.Raycast(transform.position, transform.TransformDirection(Vector3.down), out RaycastHit hit, 1f, mask);
		bool stoneStep = hit.collider.CompareTag("stone");
		...
		if (velocity >= velocityThreshold && groundCheck && groundCheck.isGrounded)
		{
			...
			{
				print("stone");
				SetPlayingMovingAudio(stoneStepAudio);
			}
			else
			{
